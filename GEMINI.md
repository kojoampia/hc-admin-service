...
- **Service Discovery & Config**: HashiCorp Consul at `localhost:8500`, registering as `hcadminservice`. **Under `prod` the app refuses to start without it** — `bootstrap-prod.yml` sets `spring.cloud.consul.config.fail-fast: true`. Under `dev` that flag is `false` in both `bootstrap.yml` and `application-dev.yml`, so an unreachable Consul _config_ server does not abort the start. What a `dev` start does when _discovery_ cannot register is settled by nothing in this repository and is deliberately not claimed here — bring Consul up either way. This said "the app refuses to start without it" flatly until 2026-09-10.
- **Messaging**: Apache Kafka via Spring Cloud Stream — an SSE fan-out to the console, verification decisions out, one plan verification back to hc-patient, and three inbound directory subscriptions from the sibling stacks. **Not roster changes.** This line said "broadcasting roster changes" until 2026-09-10, contradicting § Duty Roster Management below: nothing here publishes them and nothing reads them, no `roster` destination is declared anywhere, and the class that used to send one was deleted on 2026-09-04. The destinations are listed under Conventions — **derive them from `config/application.yml`, not from either line**
...
- **Kafka**: the declared destinations are, **outbound**, `sse-topic` (the text/plain UI fan-out, plus `messageSentEvent`), `professional-verification` (verification decisions, deliberately not on `sse-topic`), and `patient-events-plan` — item 54's answer back to hc-patient, whose topic name and `PatientEvent` envelope are **theirs**, chosen knowingly, and not to be tidied. **Inbound**: `patient-events` from hc-patient, and `hc.professional.registration` and `hc.professional.entity` from hc-professional, each in its own consumer group reading from the earliest offset. `sse-topic` is read here as well as written, by the consumer that fans out to SSE clients.

  Two corrections, both 2026-09-10. **`sse-topic` is not "both apps"** — hc-admin-gateway declares no stream functions and no bindings at all since item 40a, so this service is the only one in the stack that speaks to a broker; do not reason from a symmetry that ended. And the list stood at **two** destinations while three subscriptions and a second producer were added under it, which is how an enumeration in prose fails. There is still no `roster` topic and no `profile-updates` topic — neither string appears as a `destination:` in any `.yml` in any of the three products. **Read `config/application.yml` before assuming a binding exists**; `ConfigurationBindingTest` asserts the inbound bindings against that file, which is the only place in the suite that reads the one that ships.
